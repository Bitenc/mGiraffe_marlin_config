...
//#if ENABLED(Z_MIN_PROBE_ENDSTOP) //old
...
#define BOARD_PREINIT() { \
  OUT_WRITE(X_RESET_PIN, 1); \
  OUT_WRITE(X_SLEEP_PIN, 1); \
  OUT_WRITE(Y_RESET_PIN, 1); \
  OUT_WRITE(Y_SLEEP_PIN, 1); \
  OUT_WRITE(Z_RESET_PIN, 1); \
  OUT_WRITE(Z_SLEEP_PIN, 1); \
}
